...
object tryFinally {
  ...
  object traditional{
    ...
    /**
      ...
      */
      def withResource[R, A](resource: R)(use: R => A)(close: R => Any): A = {
        try{
          use(resource)
        } finally {
          close(resource)
  ...
  object future{
    ...
    lazy val result2Future = acquireFutureResource.flatMap{ r =>
      use(r).ensuring(releaseResource(r))
    }
  ...
  object zioBracket{
     ...
     */
    def openFile(fileName: String): Task[Source] = ZIO.effect(Source.fromFile(fileName))
    /**
    ...
    def handleFile(file: Source) = ZIO.foreach(file.getLines().toList){ line =>
      putStrLn(line)
    }
...
object zioZManaged{
   ...
   */
  val file1: ZManaged[Any, Throwable, Source] = ZManaged.make(openFile("test1.txt"))(closeFile)
  /**
  ...
  // Комбинирование
  lazy val combined: ZManaged[Any, Throwable, (Source, Source)] = file1 zip file2
  ...
  lazy val files: ZManaged[Any, IOException, List[Source]] = ZManaged.foreach(fileNames){ name =>
    file(name)
  }
   ...
   */
  lazy val r3: ZIO[Any, Throwable, List[String]] = files3.use{ l =>
    ZIO.foreach(l){ s =>
